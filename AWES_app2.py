#python -m streamlit run AWES_app2.py
...
class KiteApp:
    ...
    with Tab1:    
        ...
        def plot_graphs_energy(self):
            if 'plot_data' in st.session_state:
                fig = go.Figure()

                colors = ['blue', 'red', 'green', 'purple', 'orange', 'pink']  # Colores para los gráficos adicionales
                for idx, (fig_data, kite_area_value, min_wind_speed_value, max_wind_speed_value, data) in enumerate(st.session_state['plot_data']):
                     color = colors[idx % len(colors)]  # Asignar color cíclicamente
                     fig.add_trace(
                         go.Scatter(x=data["omega"], y =data["torque"], mode='lines',
                                    name=f'Minimum Wind Speed: {min_wind_speed_value} m/s. Maximum Wind Speed: {max_wind_speed_value} m/s. Kite area: {kite_area_value}',
                                      line=dict(color=color))
                     )
                fig.update_layout(
                    xaxis_title='Omega (RPM)',
