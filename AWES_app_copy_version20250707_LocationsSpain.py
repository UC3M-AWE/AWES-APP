#python -m streamlit run AWES_app_copy_version20250707_LocationsSpain.py
...
@st.cache_data
def load_dataset():
    return xr.open_dataset("Wind_Data.nc")

ds = load_dataset()

# Define city list and coordinates (if still needed elsewhere)
cities = [
...
city_coords = {
    ...
    "Valencia": (39.470, -0.376), "Seville": (37.389, -5.982),
    "Zaragoza": (41.649, -0.889), "Málaga": (36.721, -4.421),
    "Murcia": (37.992, -1.131), "Palma de Mallorca": (39.570, 2.650),
    "Las Palmas": (28.123, -15.436), "Bilbao": (43.263, -2.935)
}
...
# Pre-build DataFrame for city lookup (if needed)
rows = []
for city in cities:
    lat, lon = city_coords[city]
    for i, month in enumerate(months):
        wind_10m  = ds["si10"].sel(latitude=lat, longitude=lon, method="nearest").isel(valid_time=i).item()
        roughness = ds["fsr"].sel(latitude=lat, longitude=lon, method="nearest").isel(valid_time=i).item()
        geopot    = ds["z"].sel(latitude=lat, longitude=lon, method="nearest").isel(valid_time=i).item()
        altitude  = geopot / 9.80665
        rows.append({
            "City": city,
            "Month": month,
            "WindSpeed10m": wind_10m,
            "Roughness": roughness,
            "Altitude": altitude
        })
...
class KiteApp:
    ...
    def __init__(self):
        ...
        st.sidebar.image(logo, use_container_width=True)
         ...
         # --- Location Selection Branch ---
        if not st.session_state.location_confirmed:
            # Month selector
            month_sel = st.sidebar.selectbox("Select Month", options=months)
            # Folium map
            ...
            if map_result and map_result.get("last_clicked"):
                st.session_state.sel_lat = map_result["last_clicked"]["lat"]
                st.session_state.sel_lon = map_result["last_clicked"]["lng"]
                st.sidebar.write(f"Picked: {st.session_state.sel_lat:.3f}, {st.session_state.sel_lon:.3f}")
        ...
        # --- Graphing Branch (after confirm) ---
        # Use stored coordinates
        lat = st.session_state.sel_lat
        lon = st.session_state.sel_lon
        # Month selector with persistence
        month_idx_default = months.index(st.session_state.get("month_sel", months[0]))
        month_sel = st.sidebar.selectbox("Select Month", options=months, index=month_idx_default)
        st.session_state.month_sel = month_sel

        # Fetch data from NetCDF
        m_idx    = months.index(month_sel)
        wind_10m = ds["si10"].sel(latitude=lat, longitude=lon, method="nearest").isel(valid_time=m_idx).item()
        z_0      = ds["fsr"].sel(latitude=lat, longitude=lon, method="nearest").isel(valid_time=m_idx).item()
        geopot   = ds["z"].sel(latitude=lat, longitude=lon, method="nearest").isel(valid_time=m_idx).item()
        altitude = geopot / 9.80665
        h_ref    = 10.0

        st.sidebar.write(
            f"Location: {lat:.3f}, {lon:.3f} – {month_sel} | "
            f"U₁₀={wind_10m:.2f} m/s, z₀={z_0:.3f} m, h={altitude:.0f} m"
        )
        ...
        h_0 = z_0  # Roughness length Vortex data
        rmax = 200
        rmin = 100
